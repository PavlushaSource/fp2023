...
let p_char = whitespace *> char '\'' *> any_char <* char '\'' >>| fun char -> V_char char
...
let p_type =
  let parse_simple_type t err =
    ...
    | _ -> fail @@ "Unknown type in col: " ^ Int.to_string err
  in
  pos
  >>= fun col ->
...
let p_expr : expr t =
  fix (fun expr ->
    let term =
      choice [ parens expr; p_func_call expr; p_index_array expr; var_name; p_const ]
    in
...
let p_arg = p_type <* whitespace >>= fun t -> p_ident >>= fun id -> return @@ Arg (t, id)
...
let rec p_mult_assign expr1 =
  ...
  whitespace *> peek_char
  >>= function
  | Some '=' -> p_mult_assign expr2 >>= fun expr2 -> return @@ Assign (expr1, expr2)
  | Some ';' -> advance 1 *> (return @@ Assign (expr1, Expression expr2))
  | _ -> fail "expected ';' or '=' at the end"
;;
...
let p_var_decl =
  let size_arr : int t =
    ...
    >>= function
    | V_int x -> return x
    | _ -> fail @@ "size of array must be integer. Error in col: " ^ Int.to_string col
  in
  let p_type_array t =
  ...
  | t ->
    ...
    | Some '[' ->
      ...
      whitespace *> peek_char
      >>= (function
      | Some ';' -> advance 1 *> (return @@ Var_decl (t, id, Some (Expression exp1)))
      | _ -> fail "Error initialization with array")
    | Some '=' ->
      ...
      >>= (function
      | Some ';' -> advance 1 *> (return @@ Var_decl (t, id, Some (Expression exp1)))
      | Some '=' -> p_mult_assign exp1 >>= fun st -> return @@ Var_decl (t, id, Some st)
      | _ -> fail "expected ';' or '=' at the end")
...
let p_return =
  ...
  | Some _ -> p_expr >>= fun exp -> token ";" *> (return @@ Return exp)
  | _ -> fail "Error return"
;;
...
let p_for statements =
  ...
  *> (p_var_decl <|> p_assign >>| Option.some <|> (return None <* token ";"))
  ...
  <* token ")"
  <* whitespace
  >>= fun exp2 -> p_compound statements >>= fun cmd -> return @@ For (st, exp1, exp2, cmd)
;;
...
let p_func_decl statements =
  ...
  >>= function
  | Some '{' ->
    p_compound statements >>= fun cmd -> return @@ Func_def (Func_decl (t, id, argls), cmd)
...
let p_top_var =
  p_var_decl
  >>= function
  | Var_decl (idd, tp, exp) -> return @@ Top_var_decl (idd, tp, exp) <* whitespace
  | _ -> fail "ERROR"
;;
